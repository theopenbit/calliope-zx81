...
extern const unsigned char ROM81[];

#define Z80_FETCH_BYTE(address, x)                                      \
{                                                                       \
        (x) = (address)<0x4000?ROM81[(address)&0x1FFF]:RAM[(address)&(RAMSIZE-1)]; \
}

#define Z80_FETCH_WORD(address, x)                                      \
{                                                                       \
  (x) = ((address)<0x4000?ROM81[(address)&0x1FFF]:RAM[(address)&(RAMSIZE-1)]) \
         |((((address)+1)<0x4000)?ROM81[((address)+1)&0x1FFF]:                \
           RAM[((address)+1)&(RAMSIZE-1)])<<8; \
}

#define Z80_READ_BYTE(address, x) Z80_FETCH_BYTE(address, x)

#define Z80_WRITE_BYTE(address, x)                                      \
{                                                                       \
	if (((address)>=0x4000) && ((address)<(0x4000+RAMSIZE)))\
           RAM[(address)&(RAMSIZE-1)] = (x);                    \
        /*else GPIO_OUTPUT_SET(2,(x)&1);*/                       \
}

#define Z80_READ_WORD(address, x) Z80_FETCH_WORD(address, x)

#define Z80_WRITE_WORD(address, x)                                      \
{ Z80_WRITE_BYTE(address,x); Z80_WRITE_BYTE((address)+1,x>>8); }

#define Z80_INPUT_BYTE(port, highaddr, x)                                \
{                                                                       \
     unsigned char i,m;						\
        m=(highaddr);                                           \
	(x)=0xFF;						\
        if ((port)==0xFE) { for(i=0;i<8;i++)                           \
                           if (!(m&(1<<i))) (x)&=kbd_table[i];         \
      /* os_printf("In(%02x,%02x) => %02x\n",m,port,(x)); */           \
                     }                                                  \
      /*  else {os_printf("In(%02x,%02x) => %02x (%d)\n",m,port,(x),__LINE__); \
              os_printf("PC : %x - Instr : %x - opcode %x\r\n",pc, \
                        instruction,opcode); } */\
}
